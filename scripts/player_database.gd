...
func _load_data():
        ...
        # 加载转会历史球员
        var transfer_file = FileAccess.open("res://data/transfer_players.json", FileAccess.READ)
        if transfer_file:
                var json2 = JSON.new()
                if json2.parse(transfer_file.get_as_text()) == OK:
                        var transfer_players = json2.data.get("players", {})
                        for pid in transfer_players:
